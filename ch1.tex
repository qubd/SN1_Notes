...
\begin{example}\label{FirstHistogramRecap}
...
\begin{remark}
While the notation for the sample mean and population mean ($\overline{x}$ and $\mu$ respectively) is essentially universal, the notation for the sample median can vary from source to source. We'll use $\tilde{x}$ for the sample median, and have no special symbol reserved for the population median (which doesn't play an important role in the remainder of the course).
\end{remark}
%Examples
...
The idea is to remove the smallest $25\%$ and the largest $25\%$ of the data, and measure the range of whatever is left over. This is a quick way to quantify dispersion, which as with median, is not strongly influenced by the presence of outliers.

\begin{remark}
The details of the how quartiles are computed can vary from source to source, but the idea is always to partition the sorted data in four contiguous blocks of roughly the same size using three values (with the median as the central value).
\end{remark}

\begin{example}
Calculate median and IQR of the variable that takes the values below. 
$$1\ \ 1\ \ 2\ \ 4\ \ 7$$
The values are already in sorted order, so we can see the median is $\tilde{x}=2$, and the first and third quartiles are $Q_1 = \frac{1+1}{2} = 1$, and $Q_3 = \frac{4+7}{2} = 5.5$. Then $IQR = 5.5 - 1 = 4.5$.
\end{example}

\begin{example}
Computing the median and quartiles of the data in Example \ref{FirstHistogram} gives $\tilde{x} = 7$, $Q_1 = 4$, and $Q_3 = 10.5$. The smallest and largest data values, the two quartiles, and the median, taken together, form the \newterm{five-number summary}\index{Five-Number Summary} of a variable's distribution, and we can plot these five values as shown below to create a \newterm{boxplot}\index{Boxplot}.
...
Boxplots are very commonly used as visual summaries of distributions, since they're easy to construct and effectively convey key features of the distribution, in particular the center and skew (notice how the longer right-tail of the histogram in Example \ref{FirstHistogram} is visible in the boxplot above). The IQR also appears in the boxplot, as the width of the central box.

Although the IQR is useful for summarizing data, by far the most important measure of dispersion in statistical theory and practice is the variance, together with its square root, the standard deviation.
...
There are two important changes to note. First, the mean is denoted $\littlexbar$, not $\mu$, which is a reminder we're working with sample data, and second, instead of averaging as usual, by dividing by the number of terms in the sum, we decrease the divisor by one. That change is called Bessel's correction.
...
Treating the list of values $1$, $2$, $3$, $4$, $5$, $6$ as our complete population, we have
