...
Notice that in practice, it is often very difficult to obtain a random sample. It requires a complete list of all members of the population under study, and if these members are people, a way to guarantee they will volunteer the information the experimenters are interested in knowing. If you read any corporate software license agreement, you will almost certainly see a section which affirms all your user data is accessible for studies carried out by the company that developed the software. This ensures the company can obtain high quality samples from the population of its users.
...
There is nothing incorrect about either of the two histograms, it is simply a quirk of the way the axis was divided into bins that results in the difference in appearance. This illustrates how it's sometimes possible to build intentional bias into a histogram.
\end{example}
...
It's often useful to quantify the notion of the center of a distribution by producing a single `typical' or `central' value. The definition below gives three ways to do this.

\begin{definition}
Let $x_1, x_2,\,\dots\,, x_n$ be the values of a statistical variable that occur across the $n$ individuals in the dataset.
\begin{itemize}
\item The \newterm{mean}\index{Mean} value of the variable is given by $\frac{1}{n}\sum_{i} x_i = \frac{1}{n}(x_1 + x_2 + ... + x_n)$.
\item The \newterm{median}\index{Median} value of the variable is obtained by ordering $x_1, x_2, ...\,, x_n$ smallest to largest, then taking the value in the middle of the resulting list if $n$ is odd, and the mean of the two values nearest the middle if $n$ is even.
\item The \newterm{mode}\index{Mode} is the value of the variable which occurs most frequently. Note that there may be more than one mode, and in these cases we say the variable is multimodal.
\end{itemize}
\end{definition}
...
Note that these three measures of centre correspond to three levels or measurement. The mean is defined at the interval level and above, the median is defined at the the ordinal level and above, and the mode is always defined, even at the nominal level.
...
This is not a quirk that happens because of the particular choice of labels on the balls. In general, regardless of the distribution of values, the median minimizes the average absolute error and the mean minimizes the average squared error.

\begin{example}
In the preceding section, we constructed the histogram shown below, which summarizes the distribution of the variable whose values are:
$$2\ \ 3\ \ 3\ \ 4\ \ 5\ \ 5\ \ 6\ \ 7\ \ 7\ \ 7\ \ 7\ \ 7\ \ 7\ \ 8\ \ 8\ \ 9\ \ 9\ \ 10\ \  11\ \ 11\ \ 11\ \ 12 \ \ 14.$$
\begin{center}
\begin{tikzpicture}[scale=0.8]
    \begin{axis}[
...
\end{tikzpicture}
\end{center}
There are $23$ values in the ordered list, so the median is the $12^{th}$ value, which is a 7, while the mean is $\frac{1}{23}(2 + 3 + 3 + ... + 14) = 7.57$. Notice that the mean is a little bit larger than the median. When the distribution of a variable has a longer tail on one side, the mean is pulled towards the tail more than the median.
\end{example}
...
\begin{example}
Mr.\,Brown's grade three class has fifteen students and the mean height is 117\,cm. Mrs.\,Green's grade three class has twelve students and the mean height is 122\,cm. What is the mean height in the combined group of twenty-seven students?
...
The example above, and the one below, are typical of more involved problems about means, which generally require you to pass back and forth between means and sums. You should get used to doing this, as the same idea will reappear later in the course in problems involving the Central Limit Theorem REF!.

\begin{example}
Mr.\,Brown's grade three class has fifteen students and the mean height is 117\,cm. If a new student who is 114\,cm tall joins the class, how will the mean change?
...
In later parts of the course, it will be extremely important to distinguish the mean of a population from the mean of a random sample taken from that population. The greek letter $\mu$ is used for the mean of a population (a parameter), while $\littlexbar$ is used for the mean of a sample (a statistic). Whenever possible, we'll try to use greek letters for parameters and latin letters for statistics.
...
\begin{warning}
The standard deviation is not the average distance from the mean. We know $\sqrt{a^2 + b^2} \neq a + b$, and similarly, the average of a sequence of terms is not equal to the square root of the average of their squares.

The actual average distance from the mean is known as the mean absolute deviation\index{Mean Absolute Deviation}, which we will not study in this course. It's a fine measure of dispersion, but does not play a central role in statistics, as the variance and standard deviation do.
\end{warning}
...
\begin{definition}\index{Sample Standard Deviation}\index{Standard Deviation! of a Sample}\index{Variance! of a Sample}\index{Bessel's Correction}\label{SampleStdDev}The (Bessel corrected) sample variance is $s^2 = \frac{1}{n-1}\sum_{i} (x_i - \overline{x})^2$ and the (Bessel corrected) sample standard deviation, $s$, is its square root.
\end{definition}

There are two important changes here. Firstly, the mean is denoted $\littlexbar$, since we're working with sample data, and secondly, instead of averaging as usual, by dividing by the number of terms in the sum, we decrease the divisor by one. This is Bessel's correction.
...
\begin{remark}
Sample variance and sample standard deviation always refer to the Bessel corrected values. When referring to the original formula in Definition \ref{VarianceDef}, we'll omit the prefix sample, or use the term population explicitly.
\end{remark}
...
Treating the values $1$, $2$, $3$, $4$, $5$, $6$ as a complete population, we have
