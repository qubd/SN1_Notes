...
\pagestyle{headings}
\chapter{Frequentist Statistics} \label{chp 5}

\section{Sampling Distributions}

Recall that a statistic is a numerical property of a sample. It's value is determined by the sample it was derived from. Therefore, we can think of a statistic as a function whose input is a sample, and whose output is a real number.
...
\noindent Above, we've constructed a table with a row for each of the $3\cdot3 = 9$ possible samples. $X_1$ and $X_2$ denote the ages of the first and second individuals in our sample, and $\overline{X} = \frac{1}{2}(X_1 + X_2)$ is their mean.

\noindent If we assume every sample is equally likely, then each of the nine rows of the table above occurs with probability $\frac{1}{9}$, and we obtain the sampling distribution below.
...
If $\overline{X}$ is the mean of a random sample taken from a population with mean $\mu$, then $E(\overline{X}) = \mu$. The average sample mean, across all samples, is precisely equal to the population mean.
\end{proposition}
...
This is a key result that will appear in the inference procedures later in the chapter, and it's a very special property of the mean which is generally not true of other statistics.
...
\begin{remark}
The notation $\sigma_{\overline{X}}$ is used for the standard deviation of $\overline{X}$, so taking the square root of each side of $\Var(\overline{X}) = \frac{\sigma^2}{n}$, we obtain $\sigma_{\overline{X}} = \frac{\sigma}{\sqrt{n}}$, which is how the result is often stated in statistics texts.
\end{remark}
...
To sum up, we've learned some important things about the \emx{distribution of means of random samples}. In any scenario where we compute the mean of a random sample, the average sample mean is precisely equal to the population mean, and the variance of the distribution of sample means is $n$ times smaller than the variance of the population. This is expressed more formally below.

\begin{keypoint}The mean of random sample, $\xbar$, taken from a population with mean $\mu$ and variance $\sigma^2$, is a random variable whose mean and variance are
...
Now we know there's a simple relationship between the mean and variance of the distribution of $\xbar$ and the mean and variance of the population we're sampling. What can we say about how the shape of the distribution of $\xbar$ compares to the shape of the population distribution?

To illustrate, let's take the continuous random variable with density $f_X(x) = \frac{1}{2}e^{-\frac{1}{2}x}$ on $[0,\infty)$.

\begin{remark}
This random variable $X$ is known as an exponential random variable with parameter $\frac{1}{2}$, which we can abbreviate $X \sim \Exponential(\frac{1}{2})$. Exponential distributions are studied in detail in the Probability \& Random Variables option course, 201-PRV. For our purposes, it's simply a convenient distribution to use to illustrate the main result in this section. Note that $E(X) = \int_{0}^{\infty}\frac{1}{2}xe^{-\frac{1}{2}x}\,dx = 2$.
\end{remark}

We'll approximate the sampling distribution of $\xbar$ by taking ten thousand samples, computing the mean of each, and constructing a histogram of the resulting values. We'll start with samples of size one.
...
The result is not surprising. The distribution on the right (means of samples of size one) has the same shape as the distribution on the left (the distribution of the population those samples were drawn from). The mean of a sample consisting of a single observation is just that observation itself, so we're simply collecting ten thousand realizations of $X$, and constructing a histogram.
...
\end{minipage}
\end{center}
If we continue increasing the sample size, taking ten thousand random samples of larger and larger sizes from the distribution of $X$, how will the distribution of the sample mean $\xbar$ change?
\begin{center}
    \begin{minipage}{.5\textwidth}
        \centering
  \begin{tikzpicture}[scale = 0.8]
...
In the last section, we saw that $E(\xbar) = \mu$ and $\Var(\xbar) = \frac{\sigma^2}{n}$. These results are visible in the graphics above. Each distribution of $\xbar$ is centered at 2, which is the mean of the distribution the samples were drawn from, and the dispersion of the distribution of $\xbar$ decreases as the sample size grows (note that $\frac{\sigma^2}{n}$ decreases as $n$ increases).

Moreover, there is a clear trend in the shapes of the sampling distributions. As the sample size grows, the distribution of $\xbar$ looks less like an exponential distribution, and more like a Gaussian distribution.

This phenomenon is why the Gaussian distribution plays a central role in statistics. If some quantity is obtained by averaging a random sample, and the sample size is large enough, then the distribution of that quantity must be approximately Gaussian \emph{regardless of the shape of the distribution the sample was drawn from}.
...
Thus, to answer this question, we need to find the probability of obtaining a sample of twenty values with a mean $\xbar$ higher than $\frac{2}{5}$. We know from Example \ref{MeanVarianceUnitInterval} that the mean and standard deviation of the distribution our sample values are drawn from are $\mu = \frac{1}{2}$ and $\sigma = \frac{1}{\sqrt{12}}$.

By the central limit theorem, the distribution of $\xbar$ is approximately Gaussian, with a mean of $\mu = \frac{1}{2}$ and a standard deviation of $\frac{\sigma}{\sqrt{n}} = \frac{1}{\sqrt{12}\sqrt{20}} = \frac{1}{\sqrt{240}}$, or more concisely, $\xbar \sim AG(\frac{1}{2}, \frac{1}{\sqrt{240}})$.
...
As a rule of thumb, we'll require a sample size of thirty or more to treat the distribution of $\xbar$ as if it's Gaussian. There is nothing mathematically significant about this value, or slightly different values you might find in other sources. We're simply establishing a clear, yet arbitrary, baseline. 

\begin{remark}
If the population distribution is known to be Gaussian, any sample size will suffice. An important property of Gaussian distributions is that the distribution of the mean of any number of observations from a Gaussian distribution is again Gaussian.
\end{remark}
...
\begin{keypoint}
It's very important to know the hypotheses of the central limit theorem, because all the procedures that follow in this chapter treat the mean of a sample as a realization of a Gaussian random variable, which is justified as long as these hypotheses hold.
\begin{itemize}
\item The sample data was obtained via random sampling.
\item The sample data came from a distribution with a well-defined mean and variance.
...
\section{Confidence Interval for a Population Mean}\index{Confidence Interval! for a Mean}\label{ConfIntervalIntro}

\subsection*{Estimating Average Age}

Suppose we're interested in knowing the mean age of students at Champlain College. To estimate it, we take a random sample of 50 students and record their ages. It turns out that the mean age of students in the sample is $18.3$ years.
...
\begin{proposition} Let $\xbar$ and $S$ be the sample mean and (Bessel-corrected) sample standard deviation in a random sample of $n$ values drawn from a Gaussian distribution with mean $\mu$. Take $t^*$ such that $P(-t^* < T < t^*) = C$, and let $E = t^*\frac{S}{\sqrt{n}}$. Then $P(\xbar - E < \mu < \xbar + E) = C$.
\end{proposition}
\begin{proof}
We can repeat the derivation of the margin of error formula from earlier in this section, but with $T$ in place of $Z$. By definition of $t^*$, with probability $C$ we have
\begin{gather*}
-t^* < T < t^* \\
...
When Bob is checking how strong the experimental evidence against Charles' claim is, what he wants to know is how far into the left tail of a 90\% free thrower's distribution the result lies. Is what he observed a routine occurrence for a 90\% free throw shooter, or should it almost never happen?
...
\begin{example}
A bicycle component manufacturer claims the mean diameter of crank axles they produce is $24.98$\,mm with a standard deviation of $0.05$\,mm. In a random sample of thirty-eight axles that were measured very accurately, the mean diameter was $24.968$\,mm. Is this enough evidence to conclude the mean diameter of axles the manufacturer is producing is not $24.98\,mm$, at the 5\% significance level?

In this example, we have a \newterm{two-sided alternative hypothesis}. We're looking for evidence that the mean axle diameter \textbf{is not} $24.98$\,mm. Contrast this with the last example, where we had a \newterm{one-sided alternative hypothesis}. We were looking for evidence the mean temperature was \textbf{above} $26.2^{\circ}$\,C.
...
\end{tikzpicture}
\end{center}
Consulting a table of values for the T-distribution, we can find the critical value which produces a tail area of 5\% is $t^* = -1.796$. Therefore, the P-value in our test is (much) larger than 5\%. Using a computer to explicitly compute the area gives $P = 0.234$.
...
\begin{remark}
A useful analogy here is a jury trial. The defendant should be deemed guilty only if there's sufficient evidence, so $H_0$ is innocence and $H_a$ is guilt. In this context, convicting an innocent defendant is a Type I error, and acquitting a guilty defendant is a Type II error.
\end{remark}

\begin{example}
Sam has two coins. One is fair, and the other has tails on both sides. He selects one of the coins, without telling which, then flips it five times. Compute the probability of each type of error when testing $H_0: p = \frac{1}{2}$ against $H_a: p = 1$ at the 5\% significance level, where $p$ denotes the probability of tails on the coin.

If the coin is fair ($H_0$ is true), a Type I error will occur when we obtain enough evidence to convince us the coin has two tails. The probability a fair coin shows tails on all five flips is $(\frac{1}{2})^5 \approx 0.03$, and the probability it shows four tails and one head is about $16\%$ (using a binomial distribution with $n=5$ and $p=\frac{1}{2}$). Thus, to reject $H_0$ and make a Type I error, we need to observe five tails, so $P = 0.03 < 0.05$. Thus, the Type I error rate is about 3\%.

If the coin has two tails ($H_a$ is true), a Type II error will occur when we don't obtain enough evidence to convince us the coin has two tails. As we saw above, the probability a fair coin shows tails on all five flips is $(\frac{1}{2})^5 \approx 0.03$, but we observe this result every time the coin with tails on both sides is chosen, and hence we will always reject $H_0$ and come to the correct conclusion. Thus, the Type II error rate is 0\%.
\end{example}

In the example above, our alternative hypothesis specified a precise value of the unknown parameter $p$ (this is known in the business as a simple hypothesis). In the tests we've developed, we don't have this luxury. The alternatives in the $Z$ and $T$-tests specify a range of possible values (these are known in the business as composite hypotheses). To understand how we can compute the error rates in these cases, consider a one-sample $Z$-test of $H_0: \mu = \mu_0$ against $H_a: \mu > \mu_0$, at the 5\% significance level, as usual.
...
Now if the null hypothesis $H_0: \mu = \mu_0$ is true, then these two distributions are identical, and the correct decision is to not reject $H_0$. We'll incorrectly reject $H_0$ (a Type I error) when the area to the right of the test statistic (the $P$-value) is under $5\%$, which happens when our sample is among the $5\%$ highest samples, in terms of the sample mean.
...
The problem comes when the null hypothesis is false. If the population mean $\mu$ is higher than $\mu_0$, the correct decision is to reject $H_0$. In this situation, a Type II error will occur if the $P$-value is above $5\%$. When we select a random sample and compute its mean, we get a realization of the true distribution of $\xbar$ (the blue distribution), then we take that realization $\littlexbar$ and compute the $P$-value using the null distribution of $\xbar$ (the dashed black distribution). 

In the figure below, the crosshatched area under the null distribution is $0.05$, the area coloured green is the probability of correctly rejecting $H_0$ (obtaining a sample which results in $P < 0.05$), and the area coloured red is the probability of not rejecting $H_0$ (obtaining a sample which results in $P > 0.05$).
...
The probability of not rejecting $H_0$ when it's true is the \newterm{confidence}\index{Confidence Level! of a Hypothesis Test} level of the test, denoted $1-\alpha$, and the probability of rejecting $H_0$ when it's false is the \newterm{power}\index{Power! of a Hypothesis Test} of the test, denoted $1- \beta$. In the jury trial analogy, 100\% confidence means no innocent defendant is concvicted, while 100\% power means no guilty defendant is acquitted.
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Decision} & \textbf{$H_0$ is True} & \textbf{$H_a$ is True} \\
...
In practice, a hypothesis test is testing for the presence of some effect or change (the coin is no longer fair, the treatment lowers mean time to recovery, etc), and the null hypothesis is there is no such effect. With this is mind, rejecting $H_0$ means the test returned a positive result (the test detected an effect), and not rejecting means the test returned a negative result (the test did not detect an effect).

In the table above, the top-left and bottom right cells correspond to a true negative (concluding there is no effect when no effect exists), and a true positive (concluding there is an effect when an effect exists). The top-right cell, where a Type II error occurred, is a \newterm{false negative}. The test concludes no effect exists when in fact there is one. The bottom-left cell, where a Type I error occurred, is a \newterm{false positive}. The test concludes an effect exists when in fact there is none.

\begin{remark}
In medicine, the terms \newterm{sensitivity}\index{Sensitivity} and \newterm{specificity}\index{Specificity} are used for power $1-\beta$ and confidence $1-\alpha$ respectively. A sensitive test returns a positive result even for very small effects (has high power), and a specific test only returns a positive result when the effect is really there (has high confidence).
\end{remark}

%As we increase the confidence level (that is, decrease the significance level) of the test, the Type I error rate ($\alpha$) shrinks, but the Type II error rate ($\beta$) grows.
...
A hypothesis test is an assessment of the strength of evidence for an effect (potentially in a certain direction depending on the alternative hypothesis) in sample data, and the power of the test is the probability of detecting an effect when an effect actually exists. When we reject the null hypothesis and conclude that we've seen sufficient evidence for an effect, it's common to say that our experiment has produced a \newterm{statistically significant}\index{Statistical Significance} result.

\begin{remark}
Notice that as the effect size increases, and $\mu$ moves farther away from $\mu_0$, the power $1-\beta$ grows. As the sample size increases, and the variance of both distributions decreases, $1-\beta$ grows. The power of the test is an increasing function of both the effect size and the sample size.
\end{remark}
...
$$\begin{aligned}1-\beta &= P(\xbar < 4.30) \\[0.5ex] &= P(Z < \textstyle\frac{4.30 - 4}{\frac{3}{\sqrt{50}}}) \\ &= P (Z < 0.71) = 0.7611\end{aligned}$$
\end{example}
...
Conversely, we can undermine the scientific process by continuing to replicate an experiment until achieving significance (when there is actually no effect, this will happen by chance in 5\% of experiments done at the 5\% significance level), and then denying the experiments that did not achieve significance ever happened. This practice is known as \emph{$P$-hacking}, and is a real problem in modern scientific practice, where often there is only an incentive to publish experiments that are novel (never been done before), and have achieved significance. Attempts have been made to address this problem by giving researchers incentives to replicate the experiments of others, and places to publish experiments that did not achieve significance.
...
\end{tikzpicture}
\end{center}
Consulting a table of values for the chi-square distribution, we can find the critical value which produces a tail area of 5\% is about $x^* = 11$. Therefore, the P-value in our test is (much) larger than 5\%. Using a computer to explicitly compute the area gives $P = 0.366$.
...
\end{tikzpicture}
\end{center}
Consulting a table of values for the chi-square distribution, we can find the critical value which produces a tail area of 10\% is $x^* = 4.605$. Therefore, the P-value in our test is (much) larger than 10\%. Using a computer to explicitly compute the area gives $P = 0.153$.
...
\end{tikzpicture}
\end{center}
Consulting a table of values for the chi-square distribution, we can find the critical value which produces a tail area of 5\% is $x^* = 3.841$. Therefore, the P-value in our test is smaller than 5\%. Using a computer to explicitly compute the area gives $P = 0.037$.
