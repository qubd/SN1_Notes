...
In the table above then, the top-left and bottom right cells, labelled with $1-\alpha$ and $1-\beta$ respectively, correspond to a true negative (concluding there is no effect when no effect exists), and a true positive (concluding there is an effect when an effect exists).

The top-right cell, where a Type II error occurred, is a \newterm{false negative}. The test concludes no effect exists when in fact there is one. The bottom-left cell, where a Type I error occurred, is a \newterm{false positive}. The test concludes an effect exists when in fact there is none.
